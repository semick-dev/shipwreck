...
from ship import stage_files, ShipContext, create_artifact, write_to_storage
...
def test_push_files_in_single_directory(is_live):
    ...
    additional_contents = os.path.join(staged_path, "tests", "recordings")
    initial_contents = os.listdir(additional_contents)

    assert len(initial_contents) == 2
...
def test_push_stage_files_in_multiple_directories(is_live):
    ...
    files = context.get_recordings_files()
    staged_path = stage_files(context, files)
    additional_contents = os.path.join(staged_path, "azure-data-tables", "tests", "recordings")
    initial_contents = os.listdir(additional_contents)

    assert len(initial_contents) == 1


def test_push_create_artifact_in_multiple_directories(is_live):
    recording_json = """{
        "configuration": {
            "assets-prefix-path": "recordings/", 
            "blob_prefix": "sdk/tables/",
...
def test_push_create_artifact_in_multiple_directories_with_non_json_present(is_live):
    recording_json = """{
        "configuration": {
            "assets-prefix-path": "recordings/", 
            "blob_prefix": "sdk/tables/",
...
def test_push_create_artifact_empty_directory(is_live):
    recording_json = """{
        "configuration": {
            "assets-prefix-path": "recordings/", 
            "blob_prefix": "sdk/tables/",
...
def test_push_write_to_storage_multiple_files(is_live):
    recording_json = """{
        "configuration": {
            "assets-prefix-path": "recordings/", 
            "blob_prefix": "sdk/tables/",
